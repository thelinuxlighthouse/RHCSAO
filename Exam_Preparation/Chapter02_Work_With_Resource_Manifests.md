...
- Understand the differences betweenimperative and declarative resource management
...
- **Imperative**: Direct commands that perform an action immediately, such as `oc run nginx --image=nginx`. These are useful for quick tasks but do not persist intent.
...
### Server-Side vs. Client-Side Application

When you run `oc apply -f manifest.yaml`, the manifest is sent to the API server, which validates the resource, applies ownership tracking via annotations, and creates or updates the object. This server-side model enables:

- **Three-way merge**: The API server compares the current state, the previously applied state (stored in annotations), and the new desired state
- **Last-applied-configuration tracking**: OpenShift remembers what was last applied, enabling safe re-application
- **Conflict detection**: Multiple managers can apply changes to the same resource without overwriting each other
...
Secrets store sensitive data such as passwords, tokens, and keys. They are base64-encoded and stored in etcd. Secrets can be mounted as volumes or exposed as environment variables in pods. OpenShift supports multiple Secret types:
...
User runs `oc apply -f manifest.yaml`
        |
        v
oc client validates YAML syntax locally
        |
        v
...
5. For ConfigMaps, updates are propagated within a configurable delay (default ~30 seconds)
6. Secrets are updated more promptly due to their sensitivity
...
This recursively applies all `.yaml` and `.yml` files in the directory.
...
```bash
oc new-app registry.redhat.io/ubi9/nginx-124~ --name=nginx-app
```
...
oc kustomize ./path/to/overlay/ --enable-helm
```
...
- Restart the pod if the ConfigMap was updated after pod creation (though ConfigMap updates propagate automatically, Secrets require a restart for some mount types)
...
- Use `--continue` flag to apply remaining resources even if one fails (use with caution)
...
- Enable etcd encryption at rest for additional Secret protection
...
- Use `commonLabels` to tag resources by environment or team
...
- Always define `resources.requests` and `resources.limits` in production deployments
...
In strategic merge patches, lists are replaced entirely unless they have a `patchStrategicMerge` key defined. Container lists in pod specs are merged by the `name` field, but arbitrary lists may be replaced. Test patches with `oc kustomize` before applying.
...
| `commonLabels` | Labels applied to all resources and selectors |
...
11. The `data` field requires base64-encoded values. The `stringData` field accepts plaintext values, which the API server automatically base64-encodes when storing the Secret. Both fields cannot be used for the same key simultaneously.
...
14. Add a `commonLabels` section to the `kustomization.yaml` file:
```yaml
commonLabels:
