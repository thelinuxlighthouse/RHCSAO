...
> **Edition note:** This chapter is reviewed for OpenShift Container Platform 4.18, the version currently named for EX280. Commands that require an external chart repository or image registry also require network access and valid credentials.
...
Templates live in namespaces and can be discovered, shared, and reused. A cluster may provide shared templates in the `openshift` namespace, but the exact catalog varies by installation; always discover what exists instead of assuming a named sample is installed.
...
- Automatic backend discovery through EndpointSlice objects, using pod readiness to decide whether an endpoint is ready
...
- Routing based primarily on hostname and optional URL path
...
```
Client → Service ClusterIP → OVN-Kubernetes service handling → ready Pod IP
```

On OpenShift 4.18, OVN-Kubernetes is the default network provider and implements Service traffic in the cluster network. Do not assume a generic upstream kube-proxy mode such as IPVS when troubleshooting. First verify the Service selector, EndpointSlices, pod readiness, ports, and NetworkPolicies.
...
The Ingress Operator manages router pods in the `openshift-ingress` namespace. Their replica count, node placement, and publishing strategy depend on the `IngressController` and platform. Do not assume that a router pod runs on every node.
...
```
Pod → cluster DNS Service → DNS pods in openshift-dns
                           |
                           ├── Service records from the Kubernetes API
                           └── configured upstream resolvers for external names
```
...
```bash
# Test from an existing application pod whose image contains getent
oc exec <pod-name> -- \
  getent hosts <service-name>.<namespace>.svc.cluster.local
```

If that image has no DNS utility, use another approved diagnostic image available in the exam environment. A missing `nslookup` binary is not proof of a DNS failure.
...
```bash
# Edge - client TLS ends at the router; backend is normally plain HTTP
oc create route edge <route-name> --service=<service-name> \
  --insecure-policy=Redirect

# Passthrough - encrypted traffic reaches the TLS-capable backend unchanged
oc create route passthrough <route-name> --service=<service-name>

# Re-encrypt - client TLS ends at the router, then a new TLS connection
# is made to the backend. The destination CA must trust the backend cert.
...
- **`objects`**: An array of Kubernetes resource definitions. String fields can use `${PARAMETER_NAME}`. Use `${{PARAMETER_NAME}}` when substitution must preserve a non-string JSON/YAML type, such as an integer replica count. Do not depend on object-list order for readiness; controllers reconcile asynchronously.
...
- **`maxUnavailable: 0`**: Prevents the rolling-update strategy from deliberately making an existing ready replica unavailable before a replacement is ready. Real availability still depends on correct probes, capacity, application behavior, and infrastructure.
...
- **`wildcardPolicy: None`**: Disables subdomain wildcard routing. `Subdomain` allows hosts beneath the exact `spec.host` suffix; use it only with matching DNS and an appropriate wildcard certificate.
...
# Preview processed objects without creating them
oc process myapp-template -p APPLICATION_NAME=testapp -o yaml

# Create or update the processed objects
oc process myapp-template -p APPLICATION_NAME=testapp | oc apply -f -
...
```bash
oc exec <running-pod> -- \
  getent hosts myapp-service.myapp-project.svc.cluster.local
```

Expected: DNS resolves to the Service ClusterIP. If the application image lacks `getent`, use an approved diagnostic image available in the environment; a missing utility is not a DNS failure.
...
oc exec <running-pod> -- cat /etc/resolv.conf
...
- Compare the pod's resolver configuration with the `dns.operator/default` status and cluster DNS Service
...
- Protect external application traffic with an appropriate TLS design; choose edge, passthrough, or re-encrypt according to where TLS must terminate and how the backend certificate is managed
...
10. **"Create a Route with TLS"** → `oc create route edge <route> --service=<service> --insecure-policy=Redirect`
...
- The objectives include Helm-chart deployment. Know the basic Helm lifecycle, but verify the installed CLI, repositories, credentials, and permitted network access in the actual exam environment.
...
| Create edge TLS Route | `oc create route edge <route> --service=<svc> --insecure-policy=Redirect` |
...
9. **Edge**: TLS terminates at the router and traffic to the backend is normally plain HTTP. **Passthrough**: the router does not decrypt the TLS stream; the backend terminates it. **Re-encrypt**: client TLS terminates at the router, which opens and verifies a new TLS connection to the backend using the configured destination CA.

10. Use `oc get endpointslice -l kubernetes.io/service-name=<service-name> -o wide` and compare it with the Service selector and ready pods. `oc get endpoints <service-name>` is also useful on clusters that still expose the compatibility Endpoints object.
...
13. `maxSurge: 1` allows one extra pod above the desired count during an update. `maxUnavailable: 0` tells the rollout not to deliberately reduce ready replicas below the desired count. It reduces risk but cannot by itself guarantee zero downtime.
...
15. The OpenShift ingress controller implements Routes using managed router pods. The Ingress Operator controls their replicas, placement, certificates, and publishing strategy, so they do not necessarily run on every node. A router selects a Route by host and path and forwards traffic to the Route's Service endpoints according to its TLS mode.

---

## Verified References

- OpenShift Container Platform 4.18, *Building applications*: <https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/building_applications/index>
- OpenShift Container Platform 4.18, *Ingress and load balancing*: <https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/ingress_and_load_balancing/index>
- OpenShift Container Platform 4.18, *Networking Operators* (Ingress and DNS Operators): <https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/networking_operators/index>
